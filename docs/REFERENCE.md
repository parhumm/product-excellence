...
.venv/bin/python scripts/cli.py report RUN_ID --provider claude --out run.html
...
`report` writes the shareable standalone HTML report of a finished run — the same builder `/pex-run-brief` uses, now `engine/report.py` — through `POST /api/runs/{id}/report`, which the run page's **Export HTML** control also calls. The body carries the choice the user made there: `provider` (`codex`, `claude`, `auto`), `model` (empty or `dynamic` takes the review tier of `ai.ROUTES`), `codex_account` and `refresh`; both ids pass the mission's `model_alias` and `account_alias` validation before they reach a command line. One AI call at effort `high` reads up to six screenshots — the screens findings point at, plus the first and last — and returns only judgment: headline, ranked improvements, journey, corrections and what the run did not cover. Every number in the page comes from the record, and an item citing an evidence id the run has no screenshot for is dropped rather than rendered. The narrative is cached in the run folder as `report-narrative.json`, keyed by the provider and model asked for, while the HTML is rebuilt from the live record on every export, so later finding review is reflected without a second call. The run must be finished (409 otherwise), and the chosen worker must be signed in (409); a worker that stops answering ends the request with 504 rather than a stack trace, after ten minutes — long enough for a big run, since the call reads a whole journey at once rather than one step of it. Screenshots for a run owned by the team server are fetched into the local artifact cache first. The call is not recorded in the run's AI usage table, because a finished run's record — possibly shared and revisioned — is not rewritten by an export; the model that wrote it is named in the report's footer instead.
