...
**Export HTML** on a finished run page writes that shareable report without leaving the console. Open it, pick the AI worker, the model and, for Codex, the account — the same three choices as a mission — and press **Write report**. One AI call on your own subscription: the model reads the screens the run captured and writes the headline, the ranked improvements and the journey. A long run can keep it reading for several minutes, so the panel counts the seconds while you wait, then offers the finished report to open in a tab or to save. Every number on the page — scores, deductions, durations, addresses — comes from the run's record, not from the model, and an improvement that cites a screen the run never captured is dropped. The file is one page with the screenshots inside it, so it opens offline and asks nothing of the network. The reading is kept, so exporting again is free; the page is rebuilt each time, so a finding you reviewed since shows up in the scores. Choosing a different model, or `cli.py report ID --refresh`, writes a new reading. The team page cannot export: it has no AI worker of its own.
