...
**Working with a team.** If your team runs a server, sign in under **Settings** with the username and password for the website you work on. Missions, runs, findings, schedules and recordings then live on that server and everyone signed in sees the same results, while tests keep running on your own computer with your own Claude or ChatGPT subscription. Your connection settings, test personas and mission passwords stay on your machine, so a teammate has to set up anything local a mission needs before starting it. Signing out of the last website returns you to your own local data. Ask whoever administers the server for the address and the password.
