...
def prepared(kind,value,target,origin):
    r=copy.deepcopy(store.clean(value))
    if kind!='project':r['project_id']=target
    for key in ('mission','snapshot'):
        if isinstance(r.get(key),dict):r[key]['project_id']=target;r[key].pop('login_password',None)
...
def main(argv=None):
    ...
    with (store.DATA/'runtime.lock').open('a') as lock:
        ...
        manifest=json.loads(path.read_text()) if path.exists() else {'url':hub.CONFIG['url'],'source':args.source,'target':args.target,'origin':hub.origin(),'records':{}}
        ...
        data=[];original=[];missing=[]
        for kind in ('mission','mission_version','run','schedule'):
            for r in store.all_records(kind,local=True):
                ws=store.workspace_of(kind,r)
                if not ws:raise ValueError('Repair unresolved workspace ownership before import: '+r['id'])
                if ws!=args.source:continue
                original.append({'kind':kind,'record':r})
                value=prepared(kind,r,args.target,manifest['origin'])
                if kind=='run':
                    artifacts={}
        ...
        for kind in ('mission','mission_version','run','schedule'):
            ...
            if not path.exists() and existing:raise ValueError('Use an empty unseeded target workspace')
            if any(r['id'] not in manifest['records'] for r in existing):raise ValueError('Target contains records outside this import')
        for kind,r in data:
        ...
        print(json.dumps({'mode':'apply' if args.apply else 'dry-run','records':{kind:sum(k==kind for k,r in data) for kind in ('mission','mission_version','run','schedule')},'artifacts':sum(len(r.get('_artifacts',{})) for k,r in data),'schedules':'paused','local_resources':'Passwords, personas, custom networks and proxy credentials remain on the source machine'},indent=2))
...
def verify_artifacts(run,workspace):
    ...
    for name,want in run.get('_artifacts',{}).items():
        ...
        with hub.client().stream('GET',hub.CONFIG['url']+f'/hub/artifacts/{run["id"]}/{name}',auth=(login['username'],login['password'])) as response:
