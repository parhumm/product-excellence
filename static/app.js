...
// The last report written stays reachable until another replaces it: its links live in the page, not in a download.
let reportUrl='';
...
<div class="field full"><div class="toolbar"><button class="primary" data-export-html="${r.id}">Write report</button><small>One AI call, a minute or two. It reads the screens and ranks what to improve; every number stays the run's own.</small></div><div class="export-out"></div></div>
</div></details>`}
...
main.querySelectorAll('[data-export-html]').forEach(b=>b.onclick=()=>submitAction(b,async()=>{const box=b.closest('details'),out=box.querySelector('.export-out'),pick=n=>box.querySelector('[name='+n+']')?.value||'';
const picked=pick('export_model'),model=picked==='__custom__'?box.querySelector('#export_model_custom').value.trim():picked;
// A worker reading a long run takes minutes, so the seconds keep counting: a waiting button looks like a broken one.
const began=Date.now(),count=()=>out.innerHTML=`<p class="running-dot muted">Writing the report… ${Math.round((Date.now()-began)/1000)} s. Keep this page open.</p>`,tick=setInterval(count,1000);count();
try{const res=await fetch('/api/runs/'+b.dataset.exportHtml+'/report',{method:'POST',headers:{'X-PEX-Request':'1','Content-Type':'application/json',...(workspace?{'X-PEX-Workspace':workspace}:{})},body:JSON.stringify({provider:pick('export_provider'),model,codex_account:pick('export_codex_account')})});
...
// The report is a file to send on, so it stays a file: one link reads it here, the other saves it.
if(reportUrl)URL.revokeObjectURL(reportUrl);reportUrl=URL.createObjectURL(await res.blob());
