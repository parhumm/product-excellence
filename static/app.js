...
// A pause for the operator: recording is stopped, so the notice must say so and say how long is left.
...
return `<div class="notice"><strong>Step ${w.step} is waiting for you</strong><p dir="auto">${esc(w.instruction)}</p><p>Screen recording is stopped until you continue, and no screenshots are taken while this step waits. ${w.ask?'Type the value here and the console enters it into the field on the device, or do it on the device yourself and continue with the field empty.':'Do this on the device, then continue.'}</p><p><strong>${left?left+' seconds left':'No time left'}</strong> of the ${w.seconds} allowed. This step fails if nobody continues in time.</p><div class="toolbar">${w.ask?`<input id="ask-value" dir="auto" placeholder="${esc(w.ask)}" autocomplete="one-time-code" maxlength="64">`:''}<button class="primary" data-continue-step="${r.id}" data-step-number="${w.step}" data-token="${esc(w.token)}">Continue run</button><button class="danger" data-cancel="${r.id}">Stop run</button></div></div>`}
...
main.querySelectorAll('[data-continue-step]').forEach(b=>b.onclick=()=>submitAction(b,async()=>{
// A wrong or already used token is answered by showing what the run actually wants now, never by sending it again.
try{await api(`/runs/${b.dataset.continueStep}/continue-step`,{method:'POST',body:JSON.stringify({step:+b.dataset.stepNumber,token:b.dataset.token,value:($('#ask-value')||{}).value||''})});toast('Continuing the run')}catch(e){toast(e.message)}
await render()}));
