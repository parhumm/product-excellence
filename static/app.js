...
return `<div class="notice"><strong>Step ${w.step} is waiting for you</strong><p dir="auto">${esc(w.instruction)}</p><p>Screen recording is stopped until you continue, and no screenshots are taken while this step waits. Do this on the device, then continue.</p><p><strong>${left?left+' seconds left':'No time left'}</strong> of the ${w.seconds} allowed. This step fails if nobody continues in time.</p><div class="toolbar"><button class="primary" data-continue-step="${r.id}" data-step-number="${w.step}" data-token="${esc(w.token)}">Continue run</button><button class="danger" data-cancel="${r.id}">Stop run</button></div></div>`}
...
try{await api(`/runs/${b.dataset.continueStep}/continue-step`,{method:'POST',body:JSON.stringify({step:+b.dataset.stepNumber,token:b.dataset.token})});toast('Continuing the run')}catch(e){toast(e.message)}
await render()}));
