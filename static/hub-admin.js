...
(() => {
  ...
  const input = (name, label, type='text', value='', extra='') => `<label>${label}<input name="${name}" type="${type}" value="${escape(value)}" ${extra}></label>`;
  ...
  window.renderHubAdmin = async (el, call) => {
    ...
    try {
      ...
      el.innerHTML=`<h2>Workspace administration</h2><p class="muted">Each workspace has one shared sign-in. Changing its password requires teammates to sign in again.</p>${workspaces.map(w=>`<details><summary>${escape(w.name)} · ${escape(w.username)}</summary><p>${escape(w.url)}</p><p class="muted">Workspace ID: <code>${escape(w.id)}</code></p><form data-workspace="${escape(w.id)}"><div class="formgrid">${input('username','Username','text',w.username,'required maxlength="60" autocomplete="username"')}${input('password','New password (leave blank to keep current)','password','','minlength="15" maxlength="256" autocomplete="new-password"')}</div><p class="error" role="alert"></p><button type="submit">Change sign-in</button></form></details>`).join('') || '<p>No workspaces yet.</p>'}<details><summary>Add workspace</summary><form data-create><div class="formgrid">${input('name','Workspace name','text','','required maxlength="100"')}${input('url','Website URL','url','https://','required')}${input('allowed_domains','Allowed domains (comma-separated)')}${input('username','Workspace username','text','','required maxlength="60" autocomplete="username"')}${input('password','Workspace password','password','','required minlength="15" maxlength="256" autocomplete="new-password"')}<label><input name="seed" type="checkbox" checked> Add starter missions (uncheck for migration)</label></div><p class="error" role="alert"></p><button class="primary" type="submit">Create workspace</button></form></details>`;
  ...
  window.renderHubSettings = async (el, api, changed) => {
    try {
      ...
      el.innerHTML=`<h2>Team server</h2><p class="muted">${status.mode==='hybrid'?'Results are shared. Browser execution and AI stay on this machine.':'Local mode. Sign in to a workspace to share its results.'}</p>${Object.entries(status.logins).map(([id,w])=>`<p><strong>${escape(w.name)}</strong> · ${escape(w.username)} <button data-signout="${escape(id)}">Sign out</button></p>`).join('')}${status.admin?'<p>Admin signed in <button data-signout="admin">Sign out admin</button></p>':''}${status.pending.length?`<p class="notice">${status.pending.length} run(s) awaiting upload.</p><button data-retry-upload>Retry uploads</button>`:''}<p class="error" role="alert" data-connection-error></p><form data-hub-login><div class="formgrid">${input('url','Team server URL','url',status.url,'required placeholder="https://team.example.com"')}${input('username','Username','text','','required maxlength="60" autocomplete="username"')}${input('password','Password','password','','required maxlength="256" autocomplete="current-password"')}</div><p class="muted">Use a workspace sign-in, or “admin” to manage workspaces.</p><p class="error" role="alert"></p><button type="submit" class="primary">Sign in</button></form>${status.admin?'<section class="section" data-admin></section>':''}`;
